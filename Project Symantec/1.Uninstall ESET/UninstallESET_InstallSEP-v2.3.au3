...
_FileWriteLog($hFileClient, "Uninstalling ESET and Installing Symantec End Point - Script v2.3")
...
If @OSVersion = "WIN_XP" Or @OSVersion = "WIN_XPe" Then																	;XP mode
   ...
   ShellExecute($strCallMsiPath, '/x {950B1859-7F95-4CCF-8674-0F843B58CCAB} /qn /norestart PASSWORD=admin12345')	;Uninstall ESET v5 with Password
   Sleep(15000)
   $strCallMsiPath = _GetAppropriatePath('C:\Program Files\ESET\ESET*\callmsi.exe')										;Check if uninstall was success or failure. If success, callmsi.exe will get deleted.
   if $strCallMsiPath = "" Then
	  _FileWriteLog($hFileClient, "Uninstallation Successful...")
...
Else																													;Win7, 8, 2008, 2012 mode
   ...
   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5 WITHOUT password")
   ShellExecute($strCallMsiPath, '/x {4544542B-6BC5-4BA8-AA52-05C07A1D59F1} /qn /norestart')						;Uninstall ESET v5 without password
   Sleep(15000)
   $strCallMsiPath = _GetAppropriatePath('C:\Program Files\ESET\ESET*\callmsi.exe')										;Check if uninstall was success or failure. If success, callmsi.exe will get deleted.
   if $strCallMsiPath = "" Then
	  _FileWriteLog($hFileClient, "Uninstallation Successful...")
   ...
   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5 WITH password")
   ShellExecute($strCallMsiPath, '/x {4544542B-6BC5-4BA8-AA52-05C07A1D59F1} /qn /norestart PASSWORD=admin12345')		;Uninstall ESET v5 with Password
   Sleep(15000)
   $strCallMsiPath = _GetAppropriatePath('C:\Program Files\ESET\ESET*\callmsi.exe')										;Check if uninstall was success or failure. If success, callmsi.exe will get deleted.
   if $strCallMsiPath = "" Then
	  _FileWriteLog($hFileClient, "Uninstallation Successful...")
