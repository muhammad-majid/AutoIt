...
Func InstallSymantecEndPoint()
   ...
   If @OSArch = "X86" Then
	  $iCopyResult = FileCopy("\\192.168.41.249\ES_Softwares\SymantecEndPoint\SEP_32bit_v12.1.6608.6300.exe", @TempDir & "\SEP32\", $FC_OVERWRITE + $FC_CREATEPATH)
	  if $iCopyResult <> 0 Then
		 ...
		 $iPID = ShellExecute(@TempDir & "\SEP32\SEP_32bit_v12.1.6608.6300.exe")				;41.249 = eisf082
	  EndIf
   ElseIf @OSArch = "X64" Then
	  $iCopyResult = FileCopy("\\192.168.41.249\ES_Softwares\SymantecEndPoint\SEP_64bit_v12.1.6608.6300.exe", @TempDir & "\SEP64\", $FC_OVERWRITE + $FC_CREATEPATH)
	  if $iCopyResult <> 0 Then
		 _FileWriteLog($hFileClient, "Copying successful...")
		 _FileWriteLog($hFileClient, "Executing Symantec End Point Client 64bit...")
		 $iPID = ShellExecute(@TempDir & "\SEP64\SEP_64bit_v12.1.6608.6300.exe")				;41.249 = eisf082
	  EndIf
   EndIf
...
_FileWriteLog($hFileClient, "Uninstalling ESET and Installing Symantec End Point - Script v3.5")
