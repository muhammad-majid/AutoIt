...
Func InstallSymantecEndPoint()
   ...
   If @OSArch = "X86" Then
	  ...
	  $iPID = ShellExecute("\\192.168.41.249\ES_Softwares\SymantecEndPoint\SEP_32bit_v12.1.6608.6300.exe")				;41.249 = eisf082
   ElseIf @OSArch = "X64" Then
	  _FileWriteLog($hFileClient, "Executing SymantecEndPoint Client 64bit...")
	  $iPID = ShellExecute("\\192.168.41.249\ES_Softwares\SymantecEndPoint\SEP_64bit_v12.1.6608.6300.exe")				;41.249 = eisf082
   EndIf
...
_FileWriteLog($hFileClient, "Uninstalling ESET and Installing Symantec End Point - Script v3.0")
...
_FileWriteLog($hFileClient, "Attempting to Uninstall ESET v4a WITHOUT password..")
...
_FileWriteLog($hFileClient, "Attempting to Uninstall ESET v4b WITHOUT password..")
ShellExecute($strCallMsiPath, '/x {2EEBAC31-3EEF-4118-91CB-1A286A507DB2} /qn /norestart')							;Uninstall ESET v4b without Password')
CheckSuccess_ExitProcedure()

_FileWriteLog($hFileClient, "Attempting to Uninstall ESET v4a WITH password..")
...
_FileWriteLog($hFileClient, "Attempting to Uninstall ESET v4b WITH password..")
...
If @OSVersion = "WIN_XP" Or @OSVersion = "WIN_XPe" Then																;XP mode
   _FileWriteLog($hFileClient, "Proceeding XP mode Uninstall...")

   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5a WITHOUT password")
   ShellExecute($strCallMsiPath, '/x {950B1859-7F95-4CCF-8674-0F843B58CCAB} /qn /norestart')						;Uninstall ESET v5 without password
   CheckSuccess_ExitProcedure()

   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5b WITHOUT password")
   ShellExecute($strCallMsiPath, '/x {24B89EC5-9058-463A-A86E-9E45A347124E} /qn /norestart')						;Uninstall ESET v5b without password
   CheckSuccess_ExitProcedure()

   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5a WITH password")
   ...
   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5b WITH password")
...
Else																												;Win7, 8, 10, 2008, 2012 mode
   _FileWriteLog($hFileClient, "Proceeding Win7, Win8, Win10, Win2008, Win2012 mode Uninstall...")
