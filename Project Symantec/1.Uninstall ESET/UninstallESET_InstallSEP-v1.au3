...
_FileWriteLog($hFileClient, "Uninstalling ESET and Installing Symantec End Point - Script v1")
...
_FileWriteLog($hFileClient, "Executing Command: " & $strCallMsiPath & ' /x {36A3719F-8A06-451A-935A-B4A5BAE77C87} /qn /norestart')
$returnCode = RunWait(@ComSpec & " /c " & $strCallMsiPath & ' /x {36A3719F-8A06-451A-935A-B4A5BAE77C87} /qn /norestart')		;Uninstall ESET v4 without password
_FileWriteLog($hFileClient, "Return Code = " & $returnCode)


_FileWriteLog($hFileClient, "Attempting to Uninstall ESET v4 WITH password..")
$returnCode = RunWait(@ComSpec & " /c " & $strCallMsiPath & ' /x {36A3719F-8A06-451A-935A-B4A5BAE77C87} /qn /norestart PASSWORD=admin12345')	;Uninstall ESET v4 with Password
_FileWriteLog($hFileClient, "Return Code = " & $returnCode)
...
If @OSVersion = "WIN_XP" Or @OSVersion = "WIN_XPe" Then																			;XP mode
   ...
   $returnCode = RunWait(@ComSpec & " /c " & $strCallMsiPath & ' /x {950B1859-7F95-4CCF-8674-0F843B58CCAB} /qn /norestart')						;Uninstall ESET v5 without password
   _FileWriteLog($hFileClient, "Return Code = " & $returnCode)

   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5 WITH password")
   $returnCode = RunWait(@ComSpec & " /c " & $strCallMsiPath & ' /x {950B1859-7F95-4CCF-8674-0F843B58CCAB} /qn /norestart PASSWORD=admin12345')	;Uninstall ESET v5 with Password
   _FileWriteLog($hFileClient, "Return Code = " & $returnCode)
Else																																;Win7, 8, 2008, 2012 mode
   ...
   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5 WITHOUT password")
   $returnCode = RunWait(@ComSpec & " /c " & $strCallMsiPath & ' /x {4544542B-6BC5-4BA8-AA52-05C07A1D59F1} /qn /norestart')						;Uninstall ESET v5 without password
   _FileWriteLog($hFileClient, "Return Code = " & $returnCode)

   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5 WITH password")
   $returnCode = RunWait(@ComSpec & " /c " & $strCallMsiPath & ' /x {4544542B-6BC5-4BA8-AA52-05C07A1D59F1} /qn /norestart PASSWORD=admin12345')	;Uninstall ESET v5 with Password
   _FileWriteLog($hFileClient, "Return Code = " & $returnCode)
EndIf
