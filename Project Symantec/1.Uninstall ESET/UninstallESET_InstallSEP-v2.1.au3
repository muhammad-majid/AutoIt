...
ShellExecuteWait($strCallMsiPath, '/x {36A3719F-8A06-451A-935A-B4A5BAE77C87} /qn /norestart')							;Uninstall ESET v4 without Password')

_FileWriteLog($hFileClient, "Attempting to Uninstall ESET v4 WITH password..")
ShellExecuteWait($strCallMsiPath, '/x {36A3719F-8A06-451A-935A-B4A5BAE77C87} /qn /norestart PASSWORD=admin12345')		;Uninstall ESET v4 with Password')
...
If @OSVersion = "WIN_XP" Or @OSVersion = "WIN_XPe" Then																	;XP mode
   ...
   ShellExecuteWait($strCallMsiPath, '/x {950B1859-7F95-4CCF-8674-0F843B58CCAB} /qn /norestart')						;Uninstall ESET v5 without password

   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5 WITH password")
   ShellExecuteWait($strCallMsiPath, '/x {950B1859-7F95-4CCF-8674-0F843B58CCAB} /qn /norestart PASSWORD=admin12345')	;Uninstall ESET v5 with Password
Else																													;Win7, 8, 2008, 2012 mode
   ...
   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5 WITHOUT password")
   ShellExecuteWait($strCallMsiPath, '/x {4544542B-6BC5-4BA8-AA52-05C07A1D59F1} /qn /norestart')						;Uninstall ESET v5 without password

   _FileWriteLog($hFileClient, "Attempting to Uninstall ESET v5 WITH password")
   ShellExecuteWait($strCallMsiPath, '/x {4544542B-6BC5-4BA8-AA52-05C07A1D59F1} /qn /norestart PASSWORD=admin12345')	;Uninstall ESET v5 with Password
EndIf
