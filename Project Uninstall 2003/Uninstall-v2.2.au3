...
For $element In $ComputerNamesProject2007
   If StringCompare(@ComputerName, $element)==0 Then
	  MsgBox(4096,"Match", "Your Computer: "& @ComputerName & " matched current element: "& $element)
   Else
	  MsgBox(4096,"Mismatch", "Your Computer: "& @ComputerName & " does not matched current element: "& $element)
   EndIf
Next
